...
export default class Feedback extends React.Component {
  ...
  updateProfile(){
    ...
    const urlmore='https://khojapp.com/api/submitfeedback?email='+this.state.email+'&name='+this.state.name+'&subject='+this.state.subject+'&message='+this.state.message;
    ...
    const {assetsLoaded} = this.state;
      ...
      <AnimatedLoader visible={this.state.spinner} overlayColor="rgba(255,255,255,0.75)" source={require(".././assets/loader.json")}
        animationStyle={{width: 100,height: 100}}speed={1}/>
                ...
                <Text style={{color:'#fff',fontFamily:customfonts,fontSize:20}}>Feedback</Text>
                <View></View>
                </View>
                ...
                <Text style={{fontFamily:customfonts,color:'#fff',marginLeft:20}}>Subject</Text>
                <TextInput value={this.state.subject} onChangeText={(subject)=>this.setState({subject})}  
                            style={styles.input} placeholder="Subject"/>
                <Text style={{fontFamily:customfonts,color:'#fff',marginLeft:20}}>Message</Text>
                <Textarea onChangeText={(message)=>this.setState({message})}  
                style={{width:'100%',fontFamily:customfonts, borderWidth:1,borderRadius:5,marginBottom:10,color:'#27FF09',
                paddingHorizontal:20,paddingVertical:8,fontSize:14,backgroundColor:'#333',borderColor:'#333'}} rowSpan={3} bordered placeholder="Message" />
                
                <TouchableOpacity onPress={()=>this.updateProfile()} 
                        style={{paddingVertical:8,alignSelf:'center',paddingHorizontal:40,marginTop:10,
                        borderRadius:20,backgroundColor:'#01cc34'}}>
                    <Text style={{fontFamily:customfonts,color:'#fff'}}>Submit</Text>
                </TouchableOpacity>
                </View>
      </ImageBackground>
    );}else {
